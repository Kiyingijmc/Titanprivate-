...
- **Strategy contract**: `src/strategies/models/silver_bullet.py` (the only approved strategy; Unicorn/ICT_OTE/CRT were removed 2026-07-12 — unapproved, all NO-GO'd or unvalidated) extends `base_strategy.py` and returns a decision dict `{signal, type, price, sl, tp}` from `on_new_candle()`. Only `on_new_candle` is used by the controller and backtester. The controller enriches data via the FeatureBus (`src/features/`, smc pack wrapping `SMCAnalyzer`/`BiasEngine`; HTF bias cached per H1 bar) and filters all signals against HTF bias. New strategies enter as Trading-OS plugins per `docs/superpowers/plans/2026-07-12-titan-v15-program-roadmap.md`.
- **Risk/sizing** (`src/risk/risk_manager.py`): position sizing is **broker-spec driven** — it uses MT5 `tick_value`/`tick_size` (asset-agnostic and correct for forex/metals/indices/crypto/oil). Specs arrive via the `HISTORY` message and are stored per-symbol by `update_symbol_specs`. **If specs have not loaded for a symbol, `calculate_lot_size` fails safe (returns 0 + logs) rather than guessing** — do not reintroduce a hardcoded per-asset fallback. `normalize_price` snaps to the broker tick grid; precision is counted via `Decimal` to survive scientific-notation ticks like `1e-05`. A second, **book-wide** fail-closed rule sits in the portfolio cap (`risk.account.max_total_open_risk_pct`, checked in `_execute_signal` via `aggregate_open_risk` + `ExposureManager.check_total_risk`): it sums risk-to-stop over open positions, Titan-placed resting pending orders (their state-DB rows — the Sync Guard corroborates PENDING rows against the heartbeat's `orders` list, so don't re-narrow `_perform_reconciliation` to positions only) and in-flight sends, and if *any one* row is un-computable (stopless, or specs never loaded) it blocks **every** symbol until that clears — and Telegrams the operator naming the offending row, because otherwise a total trading stop is indistinguishable from a quiet market. Known gap: a pending order placed *manually* in MT5 has no DB row and no `sl` in the heartbeat, so the cap cannot count it (an EA change would be needed).
