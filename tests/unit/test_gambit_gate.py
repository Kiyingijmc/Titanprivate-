...
import scripts.gambit_gate as gambit_gate
...
# Broker specs for the symbols these fixtures name.
#
# gambit_gate._cost_r reads data/specs.json, which is gitignored and
# machine-local (regenerated by scripts/cache_specs.py from a live bridge), so
# it is absent on every clean checkout -- and XAUUSD/BTCUSD live ONLY there,
# not in the module's SPECS_EXTRA fallback. Pinning the specs here keeps these
# tests independent of workspace state, and stops them from silently measuring
# whatever a given machine's cached broker file happens to hold.
#
# Values are the real FBS rows (identical to SPECS_EXTRA for US30/ETHUSD), but
# the exact numbers barely matter here: `frame()` sets risk to 100 price units
# precisely so cost stays negligible against the criterion thresholds under
# test. What matters is that they are FIXED and non-forex-shaped.
_TEST_SPECS = {
    "US30":   {"tick_value": 0.1, "tick_size": 0.01},
...
def setUpModule():
    # _SPECS is the module's lazy cache; priming it makes _load_specs() a
    # no-op, so no test here ever touches the filesystem.
    gambit_gate._SPECS = dict(_TEST_SPECS)


def tearDownModule():
    gambit_gate._SPECS = None
